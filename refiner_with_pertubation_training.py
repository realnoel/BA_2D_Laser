...
class PDERefiner(L.LightningModule):
    def __init__(self, neural_operator: nn.Module, config: dict):
        ...
        self.state_noise_sigma = config["refiner"]["state_noise_sigma"]
    ...
    def predict_next_solution(self, x, cond):
        ...
        y_noised = torch.randn(
            (x.shape[0], self.time_future, *x.shape[2:]),
            dtype=x.dtype, device=device
        )

        self.scheduler.set_timesteps(self.K_max)   # NO device=... !!!

        for k in self.scheduler.timesteps:
            k_cpu = torch.tensor(int(k), dtype=torch.long, device="cpu")

            # GPU scalar time embedding
            time = torch.tensor(float(k), dtype=x.dtype, device=device) * self.time_multiplier

            # forward pass on GPU
            x_in = torch.cat([x, y_noised, cond], dim=1)
            pred = self.neural_operator(x_in, time=time)

            # scheduler step on CPU
            pred_cpu = pred.detach().float().cpu()
            y_noised_cpu = y_noised.detach().float().cpu()

            y_noised = self.scheduler.step(pred_cpu, k_cpu, y_noised_cpu).prev_sample.to(device)
    ...
    def train_step(self, x, y, cond):
        ...
        # GPU-side ops
        x_noisy = x + self.state_noise_sigma * torch.randn_like(x)
        residual = (y - x_noisy) / self.difference_weights
        noise = torch.randn_like(residual)

        # CPU scheduler timestep
        k_cpu = torch.randint(
            0,
        ...
        # GPU time embedding
        k_gpu = k_cpu.to(device, dtype=x.dtype)
        time = k_gpu * self.time_multiplier
        ...
        # CPU diffusion ops
        y_noised_cpu = self.scheduler.add_noise(residual_cpu, noise_cpu, k_cpu)
        y_noised = y_noised_cpu.to(device)

        # forward step
        x_in = torch.cat([x_noisy, y_noised, cond], dim=1)
        pred = self.neural_operator(x_in, time=time)

        # CPU target
        torch.cuda.synchronize()
        target_cpu = self.scheduler.get_velocity(residual_cpu, noise_cpu, k_cpu)
